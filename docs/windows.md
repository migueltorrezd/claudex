...
The model aliases are `sol`, `sol-fast`, `terra`, `luna`, `5.5`, `5.4`, `mini`, `5.3`, `spark`, and `5.2`. Allowed effort values are `low`, `medium`, `high`, `xhigh`, and `max`.
...
`./scripts/doctor.ps1` checks the launcher, Claude Code, proxy executable, OAuth status, Scheduled Task state, and loopback health endpoint without making a model request. It does not print credentials.
