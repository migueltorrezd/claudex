...
'@ + "`r`n")
    ...
    Assert-OutputContainsLine $installResult.Output '  Proxy transport: http'
    ...
    Copy-Item -LiteralPath (Join-Path $repoRoot 'scripts\version.ps1') -Destination (Join-Path $tempRepoScripts 'version.ps1')
...
'@
    ...
    $managedCommand = Get-ProxyTaskScript -ProxyPath $fakeProxyPath -Port $fakePort -Transport 'http'
    ...
    # Positive upgrade path: tasks created by the first Windows release did
    # not pin CCP_CODEX_TRANSPORT, but are still safe to replace because the
    # marker, executable path, port, and command shape all match.
    $escapedFakeProxyPath = $fakeProxyPath.Replace("'", "''")
    $legacyCommand = "`$env:CCP_BIND_ADDRESS='127.0.0.1'; `$env:PORT='$fakePort'; & '$escapedFakeProxyPath' serve --port $fakePort --no-monitor"
    $legacyTask = New-FakeTask -Command $legacyCommand
    Assert-True (Test-ManagedProxyTask -Task $legacyTask -ProxyPath $fakeProxyPath) 'A legacy Claudex-managed task should be recognized for a safe transport-aware upgrade'
    ...
    $otherCommand = Get-ProxyTaskScript -ProxyPath $otherProxyPath -Port $fakePort -Transport 'http'
