...
function Get-ProxyTaskScript {
    ...
    return "`$env:CCP_BIND_ADDRESS='127.0.0.1'; `$env:CCP_CODEX_TRANSPORT='$Transport'; `$env:PORT='$Port'; & '$escapedProxyPath' serve --port $Port --no-monitor"
}
...
function Get-ProxyTaskActionSpec {
    ...
    $command = Get-ProxyTaskScript -ProxyPath $ProxyPath -Port $Port -Transport $Transport
...
function Test-ManagedProxyTask {
    ...
    $legacyCommand = "`$env:CCP_BIND_ADDRESS='127.0.0.1'; `$env:PORT='$port'; & '$($ProxyPath.Replace("'", "''"))' serve --port $port --no-monitor"
    if ($command -ceq $legacyCommand) {
        return $true
    }

    $transportMatch = [regex]::Match($command, '\$env:CCP_CODEX_TRANSPORT=''(?<Transport>http|websocket|auto)''')
    if (-not $transportMatch.Success) {
        return $false
    }
    return ($command -ceq (Get-ProxyTaskScript `
        -ProxyPath $ProxyPath `
        -Port $port `
        -Transport $transportMatch.Groups['Transport'].Value))
}
...
function Register-AndStartProxyTask {
    ...
    $actionSpec = Get-ProxyTaskActionSpec -ProxyPath $ProxyPath -Port $endpoint.Port -Transport $Transport
...
function Invoke-Install {
    ...
    if (-not (Test-CcxVersionAtLeast $claudeVersion $script:CcxMinimumClaudeVersion)) {
        Stop-Install "install: Claude Code $($script:CcxMinimumClaudeVersion) or newer is required; found $(if ($claudeVersion) { $claudeVersion } else { 'unknown' }). Run: claude update"
    }
    ...
    if (-not (Test-CcxVersionAtLeast $proxyVersion $script:CcxMinimumProxyVersion)) {
        Stop-Install "install: claude-code-proxy $($script:CcxMinimumProxyVersion) or newer is required; found $(if ($proxyVersion) { $proxyVersion } else { 'unknown' })."
    }
    ...
    $proxyTransport = $env:CCX_PROXY_TRANSPORT
    if (-not $proxyTransport) {
        $proxyTransport = Get-ConfigValue $configTarget 'CCX_PROXY_TRANSPORT'
    }
    if (-not $proxyTransport) {
        $proxyTransport = 'http'
    }
    ...
    [Console]::WriteLine("  Proxy transport: $proxyTransport")
    if ($proxyRelease) {
