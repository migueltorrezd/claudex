...
function Get-CcxConfigProxyUrl {
...
if ($claudePath) {
    Write-Pass "Claude Code: $(Get-VersionLine $claudePath)"
}
else {
...
if ($proxyPath) {
    Write-Pass "Proxy: $(Get-VersionLine $proxyPath)"
}
else {
...
$configProxyUrl = Get-CcxConfigProxyUrl -ConfigPath $configFilePath
