...
Root effort: low, medium, high, xhigh, or max.
...
function Invoke-Setup {
    ...
    if (-not $selectedUtilityModel) {
        if ($Yes.IsPresent) {
            ...
            $selectedUtilityModel = Select-SetupOption '5. Utility model for titles, token counts, and small background requests' $defaultUtilityModel @(
                'sol', 'terra', 'luna', '5.5', '5.4', 'mini', '5.3', 'spark', '5.2'
            )
        }
    }
    ...
    Assert-Effort $selectedMainEffort
    Assert-Effort $selectedBgEffort
    Assert-Effort $selectedSubagentEffort
